...
export default class SummaryTab extends Vue {
    ...
    column_key = 'offer_id'
    button_columns: column_property[] = [{'column': 'offer_id', 'column_text': 'Market Close All '}]
    ...
    cell_click(column: string, value: string, row: any) {
        ...
        switch (column) {
            case "offer_id":
                ...
                this.close_position(row);
                break;
        }

    }

    async close_position(row: any) {
        try {
            debugger;
            this.clear_error();
            this.loading = true;
            const result = await api.API.close_instrument(row['instrument'], +row['offer_id'], this.auth.userHash)
            if (result) {
                if (result.success) {
                    debugger;
